...
const LogVisitModal = ({
  ...
  const handleLogVisit = (e) => {
    ...
    if (!form.studentId) {
      alert("Please select a student before logging a visit.");
      return;
    }
    ...
    setSmsPhone(getLinkedParentPhone(selectedStudent || initialStudent));
    setShowSendSmsModal(true);
  };

  const handleSendSMSAndSave = async (phoneNumber) => {
    ...
    try {
      ...
      console.error(err);
      alert("Failed to log visit and send SMS");
      throw err;
    } finally {
      setSubmitting(false);
    }
